 ...
 * @LastEditTime: 2023-02-28 10:23:33
 * @FilePath: \hrsaas\src\views\dashboard\index.vue
  ...
  <div class="dashboard-container">
      ...
      <div>
        <div class="fl headL">
          ...
          <div class="headInfoTip">
            <p class="firstChild">{{ $t('msg') }}，{{ userInfo.username }}，祝你开心每一天！</p>
            <p class="lastChild">早安，{{ userInfo.companyName }}，祝你开心每一天！</p>
          </div>
        </div>
        <div class="fr" />
      </div>
    ...
    <el-row type="flex" justify="space-between">
      ...
      <el-col :span="13" style="padding-right:26px">
        ...
        <el-card class="box-card">
          ...
          <!-- 放置日历组件 -->
          <workCalendar :start-data="new Date()" />
        </el-card>
        <!-- 公告 -->
        <el-card class="box-card">
    ...
    <el-dialog title="收货地址" :visible.sync="dialogFormVisible">
      <el-form ref="dialogForm" :model="dialogForm" :rules="dialogRules">
        <el-form-item label="离职时间" label-width="100px" prop="data">
          <!-- <el-input v-model="dialogForm.data" autocomplete="off" /> -->
          <div class="block">
            <el-date-picker
        ...
        <el-form-item label="离职原因" label-width="100px" prop="cause">
          <el-input
            v-model="dialogForm.cause"
            type="textarea"
            :rows="2"
            autocomplete="off"
          />
...
import { getUserProcessApi } from '@/api/dashboard'
...
export default {
  ...
  data() {
    return {
      ...
      dialogRules: {
        data: [{ required: true, trigger: 'blur', message: '离职时间不能为空' }],
        cause: [{ required: true, trigger: 'blur', message: '离职原因不能为空' }]
      },
  ...
  methods: {
    ...
    async dialogSubmit() {
      ...
      await getUserProcessApi({
        exceptTime: this.dialogForm.data,
        processKey: 'process_dimission',
        processName: '离职',
        reason: this.dialogForm.cause
      })
    }
  }
}
