 ...
 * @Date: 2023-02-20 14:45:28
 * @LastEditors: 方书生 fanss1368@163.com
 * @LastEditTime: 2023-02-23 10:14:23
 * @FilePath: \hrsaas\src\views\employees\component\user-info.vue
  ...
  <div class="user-info">
    ...
    <el-form label-width="220px">
      ...
      <el-row class="inline-info" type="flex" justify="center">
        <el-col :span="12">
          <el-button type="primary" @click="saveUser">保存更新1</el-button>
    ...
    <el-form label-width="220px">
      ...
      <div class="block">
        ...
        <el-row class="inline-info" type="flex" justify="center">
          <el-col :span="12">
            <el-button type="primary" @click="savePersonal">保存更新2</el-button>
            <el-button @click="$router.back()">返回</el-button>
          </el-col>
        </el-row>
      </div>
...
import { getUserDetailByIdApi, getPersonalDetailApi, saveUserDetailByIdApi, updatePersonalApi } from '@/api/employees'
...
export default {
  ...
  methods: {
    ...
    async getUserInfo() {
      ...
      if (!this.userInfo.staffPhoto) return
      this.$refs.avatarRef.fileList = [{ url: this.userInfo.staffPhoto, upload: true }]
    },

    // 获取下半的用户信息，即用户详情信息
    async getPersonalInfo() {
      this.formData = await getPersonalDetailApi(this.userId)
      if (!this.formData.staffPhoto) return
      this.$refs.personalRef.fileList = [{ url: this.formData.staffPhoto, upload: true }]
    },

    // 保存上半用户信息
    async saveUser() {
      // 保存用户信息的时候，需要拿到上传头像组件内部的头像地址
      const fileList = this.$refs.avatarRef.fileList
      if (fileList.some(item => !item.upload)) {
        this.$message.warning('图片还没有上传完成，不能提交')
        return
      }
      await saveUserDetailByIdApi({
        ...this.userInfo,
        // 可选链，当fileList有值时就会往下走，没有的话就返回空，同理走到[0]时有值的话就去读取url，没有的话页返回空
        staffPhoto: fileList?.[0]?.url || ''
      })

      // const res = await saveUserDetailByIdApi(this.userInfo)
      // console.log(res)
      this.$message.success('提交用户基本信息成功')
    },
    ...
    async savePersonal() {
      ...
      //   id: this.userId
      // })
      // this.$message.success('提交用户个人信息成功')
    }
  }
}
</script>
