 ...
 * @Date: 2023-02-20 15:10:47
 * @LastEditors: 方书生 fanss1368@163.com
 * @LastEditTime: 2023-02-22 16:11:41
 * @FilePath: \hrsaas\src\views\employees\component\job-info.vue
      ...
      <div class="block">
        ...
        <el-form-item label="汇报对象">
          <el-select v-model="formData.reportId" filterable placeholder="请选择" class="inputW">
            <el-option v-for="item in rows" :key="item.id" :label="item.username" :value="item.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="HRBP">
          <el-select v-model="formData.hrbp" filterable placeholder="请选择" class="inputW">
            <el-option v-for="item in rows" :key="item.id" :label="item.username" :value="item.id" class="inputW" />
          </el-select>
        </el-form-item>
...
import { getJobDetailApi, updateJobApi, getJobDetailListApi } from '@/api/employees'
export default {
  name: 'JobInfo',
  data() {
    return {
  ...
  methods: {
    // 获取点击的用户信息，并填充到表格
    async getJobDetail() {
      const res = await getJobDetailApi(this.userId)
      // console.log(res)
      this.formData = res
    },
    // 获取所有用户信息
    async getJobDetailList() {
      const res = await getJobDetailListApi()
      console.log(res)
      this.rows = res
    },
    // 点击提交，更新表单
    async saveJob() {
      const res = await updateJobApi(this.formData)
      console.log(res)
      this.$message.success('提交用户岗位信息成功')
      this.getJobDetail()
    }
  }
}
